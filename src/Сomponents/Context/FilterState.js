...
const FilterState = props => {
  ...
  const lineBreakes = (array) => {
    ...
    array.forEach(function(item, index) {
      ...
      let newItem = item[15].split('УТ').join('<br>УТ').split('ORDER_DELIVERY').join('<br>ORDER_DELIVERY').split('<br>');
  ...
  const getOrders = async () => {
    ...
    const res = await fetch(`https://script.googleusercontent.com/macros/echo?user_content_key=T7y7i6t4jbtvqvMHDDCCsW6fXZL2So205s3ckkSr0I5IwVSmsDIZSkw9T8V9n2BSak4SDI-IxBtK78MQjdkuysHx_BLY85zKm5_BxDlH2jW0nuo2oDemN9CCS2h10ox_1xSncGQajx_ryfhECjZEnAI_plmDinthLNjJIU0gpnD2z84IQMmqxvIgxucISEVsL0CyUbcqbcO8RFrnOh9A3xtwM_uWZNxSE8Bt31aaWRnXQaL3V1VbDg&lib=MR7K5zq08OF7EqCJ2Y3K_BOtiPuUevpBx&key=${localStorage.param}`);
  ...
  const onName = async (text) => {
    if (text === '') {
      ...
      dispatch({type: SET_LOADING, payload: true})
      const res = await fetch(`https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?name=${text}`);
      const data = await res.json();
      const dataLineBreakes = lineBreakes(data.data)
  ...
  const onPhone = async (text) => {
    if (text === '') {
      ...
      dispatch({type: SET_LOADING, payload: true})
      const res = await fetch(`https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?phone=${text}`);
      const data = await res.json();
      const dataLineBreakes = lineBreakes(data.data)
  ...
  const onId = async (text) => {
    ...
    dispatch({type: SET_LOADING, payload: true})
    const res = await fetch(`https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?chatid=${text}`);
    const data = await res.json();
    const dataLineBreakes = lineBreakes(data.data)
  ...
  const onOrderId = async (number) => {
    if (number !== '') {
    dispatch({type: SET_LOADING, payload: true})
    const res = await fetch(`https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?ArchiveID=${number}&key=${localStorage.param}`);
    const data = await res.json();
    const dataLineBreakes = lineBreakes(data.data)
  ...
  const getCouriers = async () => {
    const res = await fetch('https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?query=users');
    const data = await res.json();
    ...
    const addCourier = async (courier) => {
      ...
      await fetch(`https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?query=newuser&phoneSet=${courier.phone}&nameSet=${courier.name}`);
      } catch (error) {
    ...
    const getArchive = async (d1, d2) => {
      ...
      const res = await fetch(`https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?ArchiveDate=${((d1-d2End) > 0) ? d2End : d1}&ArchiveDate=${((d1-d2End) > 0) ? d1 : d2End}&key=${localStorage.param}`)
      const data = await res.json();
      const dataLineBreakes = lineBreakes(data.data)
    ...
    const onDeleteCourier = async (tel) => {
      try {
        ...
        console.log(tel)
        await fetch(`https://script.google.com/macros/s/AKfycbzXebhTNiUhnUgjXLkevAlwVlN6_0pmb-xOxzyB-g3pR8qj_0A/exec?query=deluser&phoneDel=${tel}`);
        } catch (error) {
        console.error('Ошибка:', error);
        }
    }
